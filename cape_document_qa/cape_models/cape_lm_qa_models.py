...
class CapeElmoQaModel(Model):
    ...
    def set_input_spec(self, input_spec, voc, word_vec_loader=None):
        ...
        self._is_train_placeholder = tf.placeholder(tf.bool, (), name='is_train')

        if self.token_lookup:
            self._batcher = TokenBatcher(self.lm_model.lm_vocab_file)
            self._question_char_ids_placeholder = tf.placeholder(tf.int32, (batch_size, None), name='lm_q_ids_pl')
            self._context_char_ids_placeholder = tf.placeholder(tf.int32, (batch_size, None), name='lm_c_ids_pl')
    ...
    def get_placeholders(self):
        ...
        return pls + [self._cached_doc_placeholder] if hasattr(self, '_cached_doc_placeholder') else pls

    def get_predictions_for(self, input_tensors: Dict[Tensor, Tensor]):
        return self._build_elmo(input_tensors, is_prod=False)

    def get_production_predictions_for(self, input_tensors: Dict[Tensor, Tensor]):
    ...
    def _build_elmo(self, input_tensors: Dict[Tensor, Tensor], is_prod=False):
        ...
        if not is_prod:
            return self._get_predictions_for(
                is_train, q_embed, q_mask, q_lm_encoding,
    ...
    def _get_production_predictions_for(self,
                                        is_train, cached_context, question_embed,
                                        question_mask, question_lm, context_embed,
                                        context_mask, context_lm, answer
                                        ) -> Prediction:
        raise NotImplementedError()

    def encode(self, batch: List[ContextAndQuestion], is_train: bool, cached_doc=None):
        ...
        if cached_doc is not None:
            ...
            data[self._cached_doc_placeholder] = np.zeros((1, 1, self.document_embedding_dim))
...
class CapeAttentionWithElmo(CapeElmoQaModel):
    ...
    def set_input_spec(self, input_spec, voc, word_vec_loader=None):
        super().set_input_spec(input_spec, voc, word_vec_loader=word_vec_loader)
        self.document_embedding_dim = self.match_encoder.layers[0].n_out
        self._cached_doc_placeholder = tf.placeholder(tf.float32, (None, None, self.document_embedding_dim), name='cached_doc')

    def _with_lm(self, is_train, embed, lm, mask, question: bool, append: bool=False, reuse=None):
        kind_name = "question" if question else "context"
        prefix = "weight" if append else "weight_embed"
        reuse = reuse if reuse is not None else question

        if self.lm_reduce_shared is not None:
            with tf.variable_scope(prefix + "_lm", reuse=reuse):
    ...
    def _get_production_predictions_for(self, is_train, cached_context_rep,
                                        question_embed, question_mask, question_lm,
                                        context_embed, context_mask, context_lm,
                                        answer) -> Prediction:

        def _get_question_rep():
            if self.append_embed:
                question_rep = self._with_lm(is_train, question_embed, question_lm, question_mask, True, reuse=tf.AUTO_REUSE)
            else:
                question_rep = question_embed

            if self.embed_mapper is not None:
                with tf.variable_scope("map_embed", tf.AUTO_REUSE):
                    question_rep = self.embed_mapper.apply(is_train, question_rep, question_mask)
            if self.append_before_atten:
                question_rep = self._with_lm(is_train, question_rep, question_lm, question_mask, True, True, reuse=tf.AUTO_REUSE)
            return question_rep

        def _get_context_rep():
            if self.append_embed:
                context_rep = self._with_lm(is_train, context_embed, context_lm, context_mask, False, reuse=tf.AUTO_REUSE)
            else:
                context_rep = context_embed

            if self.embed_mapper is not None:
                with tf.variable_scope("map_embed", reuse=tf.AUTO_REUSE):
                    context_rep = self.embed_mapper.apply(is_train, context_rep, context_mask)

            if self.append_before_atten:
                context_rep = self._with_lm(is_train, context_rep, context_lm, context_mask, False, True, reuse=tf.AUTO_REUSE)
            return context_rep

        question_rep = _get_question_rep()
        self.question_rep = question_rep
        self.context_rep = _get_context_rep()
        context_rep = cached_context_rep
